// POST /api/upload — handle JSON/CSV upload and replace data in Supabase
...
function mapCompanyRow(row) {
  ...
  // id is auto-generated by database sequence — never pass it from CSV
  return mapped;
}
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      // Get company count
      var { count: compCount } = await sb.from('companies').select('id', { count: 'exact', head: true });

      // Get all event participation counts grouped by event_code
      var { data: epCounts } = await sb.from('event_participations')
        .select('event_code')
        .then(function(res) {
          // Manual group-by from raw rows
          var map = {};
          (res.data || []).forEach(function(r) {
            map[r.event_code] = (map[r.event_code] || 0) + 1;
          });
          return { data: map };
        });

      // Get all attendee counts grouped by event_code
      var { data: attCounts } = await sb.from('attendees')
        .select('event_code')
        .then(function(res) {
          var map = {};
          (res.data || []).forEach(function(r) {
      ...
      return res.status(200).json({
        ...
        participations: epCounts || {},
        attendees: attCounts || {},
        // Backward compat
        mfe26: (epCounts || {})['MFE26'] || 0,
        mfa26: (epCounts || {})['MFA26'] || 0,
        mfe26_attendees: (attCounts || {})['MFE26'] || 0,
        mfa26_attendees: (attCounts || {})['MFA26'] || 0
      });
    } catch (err) {
  ...
  if (req.method === 'POST') {
    try {
      ...
      if (action === 'purge') {
        ...
        } else if (target.endsWith('-attendees')) {
          var code = target.replace('-attendees', '').toUpperCase();
      ...
      // UPLOAD action — accepts JSON array (data field) or CSV text (csv field)
      const { data: jsonData } = req.body || {};

      var parsed;
      if (jsonData) {
        // JSON upload: array of objects with original key names
        if (!Array.isArray(jsonData) || jsonData.length === 0) {
          return res.status(400).json({ ok: false, error: 'JSON data must be a non-empty array' });
        }
        // Normalize keys to snake_case
        var rows = jsonData.map(function(obj) {
          var row = {};
          Object.keys(obj).forEach(function(k) {
            row[normalizeHeader(k)] = obj[k] != null ? String(obj[k]) : '';
          });
          return row;
        });
        parsed = { rows: rows };
        ...
        return res.status(400).json({ ok: false, error: 'No data provided (use data or csv field)' });
      }

      if (parsed.rows.length === 0) {
        return res.status(400).json({ ok: false, error: 'Upload has no data rows' });
      }
      ...
      if (target === 'companies') {
        ...
        var msg = 'Companies replaced: ' + inserted + ' companies from ' + parsed.rows.length + ' rows.';
        return res.status(200).json({ ok: true, message: msg });

      } else if (target.endsWith('-attendees')) {
        // Attendees upload — delete and replace for this event
        var attendeeCode = target.replace('-attendees', '').toUpperCase();
        ...
        var attRows = parsed.rows.map(function(r) {
          return {
            ...
            // Preserve both fields distinctly. invitation_status drives analytics filtering
            // (Invited / Accepted / Attended / Substituted / Declined / No-Show),
            // while attendance holds the attendance mode (In-Person / Virtual).
            invitation_status: r.invitation_status || '',
            attendance: r.attendance || '',
            accepted_date: r.accepted_date || ''
          };
        }).filter(function(r) {
          // Only two exclusions: staff rows and rows with no identifying info at all.
          // Everything else (any invitation_status, any type) is kept so the dashboard
          // can filter between pre-registration and actual attendance views.
          if (/^staff$/i.test(r.type)) { droppedStaff++; return false; }
          if (!r.first_name && !r.last_name && !r.company && !r.contact_id) {
            droppedEmpty++;
            return false;
          }
          return true;
        ...
        var attMsg = attendeeCode + ' attendees replaced: ' + attInserted + ' records from ' + parsed.rows.length + ' rows.';
        if (droppedStaff || droppedEmpty) {
          attMsg += ' (Excluded: ' + droppedStaff + ' staff, ' + droppedEmpty + ' empty rows.)';
        }
        return res.status(200).json({ ok: true, message: attMsg });

      } else {
        // Event participation upload — CSV replaces all participations for this event.
        // Target is the event code (e.g. 'mfe26', 'mfa25', 'mfe25')
        var eventCode = target.toUpperCase();
        ...
        if (unmatched.length > 0) {
          ...
          for (var i = 0; i < newCompanies.length; i += 50) {
            ...
            var { data: inserted, error: compErr } = await sb.from('companies').insert(batch).select('id, company_name');
        ...
        // Deduplicate by (company_id, event_code) — keep first occurrence
        var ready = [];
        var seenKey = {};
        mapped.filter(function (r) { return r.company_id; }).forEach(function (r) {
          var dk = r.company_id + '|' + r.event_code;
        ...
        var msg = eventCode + ' replaced: ' + inserted + ' participations from ' + parsed.rows.length + ' rows.';
        if (created > 0) {
